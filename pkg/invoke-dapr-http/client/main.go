...
func main() {
	...
	var cmd string
	var port int
	flag.IntVar(&port, "port", pkg.EchoDaprClientHttpPort, "Service http listening port")
...
func InvokeHttpServiceWithSDK(clientPort int, appID string, cmd string, text string) {
	...
	// This create a dapr runtime able to connect to sidecars and access the state stores
